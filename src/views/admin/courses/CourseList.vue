<template>
  <div>
    <section class="container my-4">
      <h4 class="my-4">Relação de todos os <strong>Cursos</strong></h4>
      ...
      <table class="table table-bordered table-striped table-hover">
        ...
        <tbody>
          <tr v-for="item in items" :key="item.id">
            ...
            <td>{{ item.teaching ? item.teaching.name : "" | uppercase }}</td>
            <td>{{ item.created_at }}</td>
            <td>
              <ButtonEdit :to="{ name: 'course-edit', params: { id: item.id } }"/>
...
export default {
  ...
  methods: {
    ...
    async getItens(page = 1) {
      ...
      let uri = `/courses?page=${page}` + query;
